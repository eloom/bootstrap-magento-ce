 ...
 * @package log4php
 */
class Eloom_Log4php_LoggerReflectionUtils {
		/** the target object */
	private $obj;
	
	/**
	 * Create a new Eloom_Log4php_LoggerReflectionUtils for the specified Object. 
	...
	public static function setPropertiesByObject($obj, $properties, $prefix) {
		$pSetter = new Eloom_Log4php_LoggerReflectionUtils($obj);
	...
	public function setProperties($properties, $prefix) {
		...
		while(list($key,) = each($properties)) {
			if(strpos($key, $prefix) === 0) {
				...
				$value = Eloom_Log4php_Helpers_LoggerOptionConverter::findAndSubst($key, $properties);
				$key = substr($key, $len);
				if($key == 'layout' and ($this->obj instanceof Eloom_Log4php_LoggerAppender)) {
					continue;
				}
