...
/**
 * Eloom_Log4php_Configurators_LoggerConfiguratorPhp class
 *
 ...
 * @since 2.0
 */
class Eloom_Log4php_Configurators_LoggerConfiguratorPhp implements Eloom_Log4php_LoggerConfigurator {
	
	public function configure(Eloom_Log4php_LoggerHierarchy $hierarchy, $url = '') {
		return $this->doConfigure($url, $hierarchy);
	}
	
	private function doConfigure($url, Eloom_Log4php_LoggerHierarchy $hierarchy) {
		if (!is_array($url)) {
 			$config = require $url;
 		} else {
 			$config = $url;
		...
		if(isset($config['threshold'])) {
			$hierarchy->setThreshold(Eloom_Log4php_Helpers_LoggerOptionConverter::toLevel($config['threshold'], Eloom_Log4php_LoggerLevel::getLevelAll()));
		}
		...
		if(isset($config['appenders'])) {
			...
			foreach($config['appenders'] as $appenderName => $appenderProperties) {
				...
				$appender = Eloom_Log4php_LoggerAppenderPool::getAppenderFromPool($appenderName, $appenderProperties['class']);
				...
				if($appender->requiresLayout()) {
					...
					if(isset($appenderProperties['layout'])) {
						...
						if(isset($appenderProperties['layout']['class']) and !empty($appenderProperties['layout']['class'])) {
							...
							$layoutClass = 'Eloom_Log4php_Layouts_LoggerLayoutSimple';
						}
						...
						if($layout instanceof Eloom_Log4php_Layouts_LoggerLayoutPattern) {
				...
				$setter = new Eloom_Log4php_LoggerReflectionUtils($appender);
		...
		if(isset($config['rootLogger'])) {
			...
			if(isset($config['rootLogger']['level'])) {
				$rootLogger->setLevel(Eloom_Log4php_Helpers_LoggerOptionConverter::toLevel($config['rootLogger']['level'], Eloom_Log4php_LoggerLevel::getLevelDebug()));
			...
			foreach($config['loggers'] as $loggerName => $loggerProperties) {
				if(is_string($loggerName)) {
					...
					if(isset($loggerProperties['level'])) {
						$logger->setLevel(Eloom_Log4php_Helpers_LoggerOptionConverter::toLevel($loggerProperties['level'], Eloom_Log4php_LoggerLevel::getLevelDebug()));
