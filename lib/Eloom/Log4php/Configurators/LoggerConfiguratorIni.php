 ...
 *
 * <p>The <i>Eloom_Log4php_Configurators_LoggerConfiguratorIni</i> does not handle the
 * advanced configuration features supported by the {@link Eloom_Log4php_Configurators_LoggerConfiguratorXml} 
 * such as support for {@link Eloom_Log4php_LoggerFilter}, 
 ...
 * <b>/home/xyz</b>. See {@link Eloom_Log4php_Helpers_LoggerOptionConverter::getSystemProperty()}
 * for details.</p>
 *
 ...
 * @since 0.5
 */
class Eloom_Log4php_Configurators_LoggerConfiguratorIni implements Eloom_Log4php_LoggerConfigurator {
	 ...
	 * </pre>
	 * 
	 * For each named appender you can configure its {@link Eloom_Log4php_LoggerLayout}. The
	 ...
	 * specifying a {@link Eloom_Log4php_Renderers_LoggerRendererObject}
	 ...
	 * ; Appender "A1" will be a Eloom_Log4php_Appenders_LoggerAppenderSyslog
	 * log4php.appender.A1=Eloom_Log4php_Appenders_LoggerAppenderSyslog
	 * 
	 ...
	 * ; A2 should be a Eloom_Log4php_Appenders_LoggerAppenderRollingFile, with maximum file size of 10 MB
	 * ; using at most one backup file. A2's layout is TTCC, using the
	 * ; ISO8061 date format with context printing enabled.
	 * log4php.appender.A2=Eloom_Log4php_Appenders_LoggerAppenderRollingFile
	 * log4php.appender.A2.MaxFileSize=10MB
	 * log4php.appender.A2.MaxBackupIndex=1
	 * log4php.appender.A2.layout=Eloom_Log4php_Layouts_LoggerLayoutTTCC
	 ...
	 * @param Eloom_Log4php_LoggerHierarchy $repository the repository to apply the configuration
	 */
	public function configure(Eloom_Log4php_LoggerHierarchy $hierarchy, $url = '') {
		$properties = @parse_ini_file($url);
		if ($properties === false) {
			$error = error_get_last();
			throw new Eloom_Log4php_LoggerException("Eloom_Log4php_Configurators_LoggerConfiguratorIni: Error parsing configuration file: ".$error['message']);
		}
		if  (count($properties) == 0) {
			trigger_error("Eloom_Log4php_Configurators_LoggerConfiguratorIni: Configuration file is empty.", E_USER_WARNING);
		}
	 ...
	 * @param Eloom_Log4php_LoggerHierarchy $hierarchy
	 */
	private function doConfigureProperties($properties, Eloom_Log4php_LoggerHierarchy $hierarchy) {
		$thresholdStr = @$properties[self::THRESHOLD_PREFIX];
		$hierarchy->setThreshold(Eloom_Log4php_Helpers_LoggerOptionConverter::toLevel($thresholdStr, Eloom_Log4php_LoggerLevel::getLevelAll()));
	 ...
	 * @param Eloom_Log4php_LoggerHierarchy $hierarchy
	 */
	private function configureRootCategory($props, Eloom_Log4php_LoggerHierarchy $hierarchy) {
	 ...
	 * @param Eloom_Log4php_LoggerHierarchy $hierarchy
	 */
	private function parseCatsAndRenderers($props, Eloom_Log4php_LoggerHierarchy $hierarchy) {
	 ...
	 */
	private function parseAdditivityForLogger($props, Eloom_Log4php_Logger $cat, $loggerName) {
		$value = Eloom_Log4php_Helpers_LoggerOptionConverter::findAndSubst(self::ADDITIVITY_PREFIX . $loggerName, $props);
		$additivity = Eloom_Log4php_Helpers_LoggerOptionConverter::toBoolean($value, true);
	 ...
	 *
	 * @param array $props array of properties
	 * @param Eloom_Log4php_Logger $logger
	 * @param string $optionKey
	 * @param string $loggerName
	 * @param string $value
	 * @return Eloom_Log4php_Logger
	 */
	private function parseCategory($props, Eloom_Log4php_Logger $logger, $optionKey, $loggerName, $value) {
		...
		if(!(empty($value) || @$value[0] == ',')) {
			...
			if('INHERITED' == strtoupper($levelStr) || 'NULL' == strtoupper($levelStr)) {
				...
				$logger->setLevel(Eloom_Log4php_Helpers_LoggerOptionConverter::toLevel($levelStr, Eloom_Log4php_LoggerLevel::getLevelDebug()));
			}
		}
	 ...
	 * @return Eloom_Log4php_LoggerAppender
	 */
	private function parseAppender($props, $appenderName) {
		$appender = Eloom_Log4php_LoggerAppenderPool::getAppenderFromPool($appenderName);
		$prefix = self::APPENDER_PREFIX . $appenderName;
		if($appender === null) {
			// Appender was not previously initialized.
			$appenderClass = @$props[$prefix];
			$appender = Eloom_Log4php_LoggerAppenderPool::getAppenderFromPool($appenderName, $appenderClass);
			if($appender === null) {
				return null;
			}
		}
		...
		if($appender->requiresLayout() ) {
			...
			$layoutClass = Eloom_Log4php_Helpers_LoggerOptionConverter::substVars($layoutClass, $props);
			if(empty($layoutClass)) {
				$layout = Eloom_Log4php_LoggerReflectionUtils::createObject('Eloom_Log4php_Layouts_LoggerLayoutSimple');
			} else {
				$layout = Eloom_Log4php_LoggerReflectionUtils::createObject($layoutClass);
				if($layout === null) {
					$layout = Eloom_Log4php_LoggerReflectionUtils::createObject('Eloom_Log4php_Layouts_LoggerLayoutSimple');
				}
			}
			
			Eloom_Log4php_LoggerReflectionUtils::setPropertiesByObject($layout, $props, $layoutPrefix . ".");
			$appender->setLayout($layout);
			
		}
		Eloom_Log4php_LoggerReflectionUtils::setPropertiesByObject($appender, $props, $prefix . ".");
		return $appender;
	}
}
