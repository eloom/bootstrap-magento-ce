 ...
 * @since 0.4 
 */
class Eloom_Log4php_Configurators_LoggerConfiguratorXml implements Eloom_Log4php_LoggerConfigurator {
	...
	<log4php:configuration threshold="all">
		<appender name="A1" class="Eloom_Log4php_Appenders_LoggerAppenderEcho">
			<layout class="Eloom_Log4php_Layouts_LoggerLayoutSimple" />
		</appender>
		<root>
			<level value="debug" />
			<appender_ref ref="A1" />
	...
	/**
	 * @var Eloom_Log4php_LoggerHierarchy
	 */
	private $repository;
	...
	/**
	 * @var Eloom_Log4php_Logger parsed Logger  
	 */
	private $logger;
	
	/**
	 * @var Eloom_Log4php_LoggerAppender parsed Eloom_Log4php_LoggerAppender 
	 */
	private $appender;
	
	/**
	 * @var Eloom_Log4php_LoggerFilter parsed Eloom_Log4php_LoggerFilter 
	 */
	private $filter;
	
	/**
	 * @var Eloom_Log4php_LoggerLayout parsed Eloom_Log4php_LoggerLayout 
	 */
	private $layout;
	 ...
	 *
	 * @param string $url
	 * @param Eloom_Log4php_LoggerHierarchy $repository
	 */
	private function doConfigure($url = '', Eloom_Log4php_LoggerHierarchy $repository)
	{
		$xmlData = '';
		if (!empty($url))
	 ...
	 * @param Eloom_Log4php_LoggerHierarchy $repository
	 */
	private function doConfigureByString($xmlData, Eloom_Log4php_LoggerHierarchy $repository)
	{
	 ...
	 * @param Eloom_Log4php_LoggerHierarchy $repository
	 */
	private function doConfigureDefault(Eloom_Log4php_LoggerHierarchy $repository)
	{
	 ...
	 * @param string $xmlData
	 */
	private function parse($xmlData, Eloom_Log4php_LoggerHierarchy $repository)
	{
	...
	private function tagOpen($parser, $tag, $attribs)
	{
		switch ($tag) {
			...
			case self::XMLNS.':CONFIGURATION':
				...
				if (isset($attribs['THRESHOLD'])) {
					...
					$this->repository->setThreshold(
						Eloom_Log4php_Helpers_LoggerOptionConverter::toLevel(
			...
			case self::XMLNS.':APPENDER':
				...
				$this->appender = Eloom_Log4php_LoggerAppenderPool::getAppenderFromPool($name, $class);
			...
			case self::XMLNS.':LAYOUT':
				$class = @$attribs['CLASS'];
				$this->layout = Eloom_Log4php_LoggerReflectionUtils::createObject($this->subst($class));
			...
			case self::XMLNS.':LOGGER':
				...
				if (!empty($loggerName)) {
					...
					if ($this->logger !== null and isset($attribs['ADDITIVITY'])) {
						$additivity = Eloom_Log4php_Helpers_LoggerOptionConverter::toBoolean($this->subst($attribs['ADDITIVITY']), true);	 
				...
				switch (end($this->state)) {
					case self::APPENDER_STATE:
						if ($this->appender !== null) {
							Eloom_Log4php_LoggerReflectionUtils::setter($this->appender, $this->subst($attribs['NAME']), $this->subst($attribs['VALUE']));
						}
						break;
					case self::LAYOUT_STATE:
						if ($this->layout !== null) {
							Eloom_Log4php_LoggerReflectionUtils::setter($this->layout, $this->subst($attribs['NAME']), $this->subst($attribs['VALUE']));
						}
						break;
					case self::FILTER_STATE:
						if ($this->filter !== null) {
							Eloom_Log4php_LoggerReflectionUtils::setter($this->filter, $this->subst($attribs['NAME']), $this->subst($attribs['VALUE']));
						}
						break;
					default:
			...
			case self::XMLNS.':ROOT':
				$this->logger = Eloom_Log4php_Logger::getRootLogger();
			...
			case self::XMLNS.':APPENDER':
				if ($this->appender !== null) {
					if ($this->appender->requiresLayout() and $this->appender->getLayout() === null) {
						...
						$this->appender->setLayout(Eloom_Log4php_LoggerReflectionUtils::createObject('Eloom_Log4php_Layouts_LoggerLayoutSimple'));
					}					
	...
	{
		return Eloom_Log4php_Helpers_LoggerOptionConverter::substVars($value);
	}
