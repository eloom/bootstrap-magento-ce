 ...
 *            See {@link Eloom_Log4php_LoggerMDC} class for more details. 
 ...
 * @since 0.3 
 */
class Eloom_Log4php_Layouts_LoggerLayoutPattern extends Eloom_Log4php_LoggerLayout {
	...
	public function setConversionPattern($conversionPattern) {
		...
		$patternParser = new Eloom_Log4php_Helpers_LoggerPatternParser($this->pattern);
	 ...
	 * @return string
	 */
	public function format(Eloom_Log4php_LoggerLoggingEvent $event) {
		$sbuf = '';
		$c = $this->head;
		while ($c !== null) {
			$c->format($sbuf, $event);
	 ...
	 * 
	 * This method is mainly used for the prepared statements of {@see Eloom_Log4php_Appenders_LoggerAppenderPDO}.
	 * 
	 ...
	 */
	public function formatToArray(Eloom_Log4php_LoggerLoggingEvent $event) {
		$results = array();
		$c = $this->head;
		while ($c !== null) {
			if ( ! $c instanceOf Eloom_Log4php_Helpers_LoggerLiteralPatternConverter) {
				$sbuf = null;
				$c->format($sbuf, $event);
				$results[] = $sbuf;
			}
