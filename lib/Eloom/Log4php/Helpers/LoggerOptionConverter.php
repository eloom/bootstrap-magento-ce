 ...
 * @since 0.5
 */
class Eloom_Log4php_Helpers_LoggerOptionConverter {
	 ...
	 * character is present, then the default {@link Eloom_Log4php_LoggerLevel}
	 ...
	 *
	 * @param string $value
	 * @param Eloom_Log4php_LoggerLevel $defaultValue
	 * @return Eloom_Log4php_LoggerLevel a {@link Eloom_Log4php_LoggerLevel} or null
	 * @static
	 */
	public static function toLevel($value, $defaultValue) {
		...
		if($hashIndex === false) {
			if("NULL" == strtoupper($value)) {
				...
				return Eloom_Log4php_LoggerLevel::toLevel($value, $defaultValue);
			}
		}
		...
		if(class_exists($clazz)) {
			...
			if(!$result instanceof Eloom_Log4php_LoggerLevel) {
				$result = $defaultValue;
			}
		} 
		return $result;
	...
	public static function findAndSubst($key, $props) {
		...
		// If coming from the Eloom_Log4php_Configurators_LoggerConfiguratorIni, some options were
		...
		//
		// if(!empty($value)) {
			return Eloom_Log4php_Helpers_LoggerOptionConverter::substVars($value, $props);
		// }
	}
	 ...
	 * the call
	 * <code>
	 * $s = Eloom_Log4php_Helpers_LoggerOptionConverter::substVars("Value of key is ${MY_CONSTANT}.");
	 * </code>
	 ...
	 * <code>
	 * $s = Eloom_Log4php_Helpers_LoggerOptionConverter::substVars("Value of inexistentKey is [${inexistentKey}]");
	 * </code>
	...
	public static function substVars($val, $props = null) {
		...
		while(true) {
			...
			if($j === false) {
				...
				if($k === false) {
					// Eloom_Log4php_Helpers_LoggerOptionConverter::substVars() has no closing brace. Opening brace
					return '';
				} else {
					$j += self::DELIM_START_LEN;
					$key = substr($val, $j, $k - $j);
					// first try in System properties
					$replacement = Eloom_Log4php_Helpers_LoggerOptionConverter::getSystemProperty($key, null);
					...
					if(!empty($replacement)) {
						...
						// x1=p1
						// x2=${x1}
						$recursiveReplacement = Eloom_Log4php_Helpers_LoggerOptionConverter::substVars($replacement, $props);
