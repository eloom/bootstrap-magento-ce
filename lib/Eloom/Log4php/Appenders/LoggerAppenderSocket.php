 ...
 */ 
class Eloom_Log4php_Appenders_LoggerAppenderSocket extends Eloom_Log4php_LoggerAppender {
	...
	public function activateOptions() {
		if(!$this->dry) {
			...
			if ($this->sp === false) {
				throw new Eloom_Log4php_LoggerException("Could not open socket to ".$this->getRemoteHost().":".$this->getPort().": $errstr ($errno)");
			}
		}
		if($this->getUseXml()) {
			$this->xmlLayout = Eloom_Log4php_LoggerReflectionUtils::createObject('Eloom_Log4php_Layouts_LoggerLayoutXml');
	...
	public function setLocationInfo($flag) {
		$this->locationInfo = Eloom_Log4php_Helpers_LoggerOptionConverter::toBoolean($flag, $this->getLocationInfo());
	} 
	...
	public function setLog4jNamespace($flag) {
		$this->log4jNamespace = Eloom_Log4php_Helpers_LoggerOptionConverter::toBoolean($flag, $this->getLog4jNamespace());
	} 
	...
	public function setPort($port) {
		$port = Eloom_Log4php_Helpers_LoggerOptionConverter::toInt($port, 0);
	...
	public function setTimeout($timeout) {
		$this->timeout = Eloom_Log4php_Helpers_LoggerOptionConverter::toInt($timeout, $this->getTimeout());
	}
	...
	public function setUseXml($flag) {
		$this->useXml = Eloom_Log4php_Helpers_LoggerOptionConverter::toBoolean($flag, $this->getUseXml());
	} 
 
	public function append(Eloom_Log4php_LoggerLoggingEvent $event) {
		if($this->sp || $this->dry) {
