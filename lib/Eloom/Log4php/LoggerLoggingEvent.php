 ...
 * @package log4php
 */
class Eloom_Log4php_LoggerLoggingEvent {
	...
	*/
	...
	/** 
	* @var Eloom_Log4php_LoggerLocationInfo Location information for the caller. 
	*/
	private $locationInfo = null;
	
	/**
	 * @var Eloom_Log4php_LoggerThrowableInformation log4php internal representation of throwable
	 */
	...
	* Eloom_Log4php_LoggerLoggingEvent are filled when actually needed.
	*
	* @param string $fqcn name of the caller class.
	* @param mixed $logger The {@link Logger} category of this event or the logger name.
	* @param Eloom_Log4php_LoggerLevel $priority The level of this event.
	...
	*/
	public function __construct($fqcn, $logger, $priority, $message, $timeStamp = null, Exception $throwable = null) {
		$this->fqcn = $fqcn;
		if($logger instanceof Eloom_Log4php_Logger) {
			$this->logger = $logger;
		...
		if ($throwable !== null && $throwable instanceof Exception) {
			$this->throwableInfo = new Eloom_Log4php_LoggerThrowableInformation($throwable);
		}
	}
	 ...
	 * @return Eloom_Log4php_LoggerLocationInfo
	 */
	public function getLocationInformation() {
		if($this->locationInfo === null) {
			...
			if(function_exists('debug_backtrace')) {
				...
				while($hop !== null) {
					if(isset($hop['class'])) {
						...
						if(!empty($className) and ($className == 'eloom_log4php_logger' or $className == 'eloom_log4php_loggercategory' or 
							strtolower(get_parent_class($className)) == 'eloom_log4php_logger' or
							strtolower(get_parent_class($className)) == 'eloom_log4php_loggercategory')) {
			...
			$this->locationInfo = new Eloom_Log4php_LoggerLocationInfo($locationInfo, $this->fqcn);
		}
	 ...
	 * thread or even on a different machine. The {@link Eloom_Log4php_LoggerNDC::get()} method
	...
	public function getNDC() {
		if($this->ndcLookupRequired) {
			...
			$this->ndc = Eloom_Log4php_LoggerNDC::get();
		}
		return $this->ndc;
	}
	...
	public function getMDC($key) {
		return Eloom_Log4php_LoggerMDC::get($key);
	}
	...
	public function getRenderedMessage() {
		if($this->renderedMessage === null and $this->message !== null) {
			if(is_string($this->message)) {
				...
				// But in contrast to log4j, in log4php there is only have one Eloom_Log4php_LoggerHierarchy so there is
				// no need figure out which one is $this->logger part of.
				// TODO: Logger::getHierarchy() is marked @deprecated!
				$repository = Eloom_Log4php_Logger::getHierarchy();
	...
	public function getTime() {
		...
		$eventStartTime = (float)Eloom_Log4php_LoggerLoggingEvent::getStartTime();
	...
	/**
	 * @return mixed Eloom_Log4php_LoggerThrowableInformation
	 */
...
Eloom_Log4php_LoggerLoggingEvent::getStartTime();
